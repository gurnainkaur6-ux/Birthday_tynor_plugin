...
(function () {
  const getBaseUrl = () => (typeof window.APP_BASE_URL !== 'undefined' ? window.APP_BASE_URL : '');
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.sidebar-link').forEach(function (link) {
      ...
      if (!href) return;
      const linkPath = href.replace(baseUrl, '').split('?')[0];
    ...
    if (searchInput && searchResults) {
      ...
      searchInput.addEventListener('input', function () {
        ...
        timer = setTimeout(function () {
          fetch(baseUrl + '/employees/search_ajax.php?q=' + encodeURIComponent(q), {
            ...
            .then(function (data) {
              if (data.employees && data.employees.length > 0) {
                ...
                data.employees.forEach(function (emp) {
                  html += '<tr>'
                    + '<td><code>' + escapeHtml(emp.emp_code) + '</code></td>'
                    + '<td>' + escapeHtml(emp.emp_name) + '</td>'
                    + '<td class="text-muted small">' + escapeHtml(emp.department_name || '—') + '</td>'
                    + '<td class="small">' + escapeHtml(emp.dob || '—') + '</td>'
                    + '<td><a href="' + baseUrl + '/employees/view.php?code=' + encodeURIComponent(emp.emp_code) + '" class="btn btn-xs btn-outline-primary btn-sm py-0">View</a></td>'
                    + '</tr>';
                });
